...
- **Gestión de la firma.** El criterio pide el SRS con la firma de aprobación del docente-director, y es el único punto de la rúbrica que **no depende del equipo**. Se declara aquí lo que sí se puede acreditar:
  - **El entregable está terminado.** `docs/requisitos/SRS-v1.3.0.pdf`, 24 páginas (incluye la página de aprobación), compila con 0 errores y su texto coincide con una compilación nueva (comprobado en Piso 2). Incluye su página de firma y no tiene observaciones pendientes de resolver por el equipo.
