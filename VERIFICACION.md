...
```
- **DOI del software corregido (P4).** `CITATION.cff` declaraba la versión 1.2.0 con el DOI `10.5281/zenodo.22740480`, que **resuelve a la v1.0.1** (14-sep), un depósito anterior que además lista al tercer integrante entre los creadores. Quien siguiera ese identificador descargaba código que no era el evaluado. Se sustituye por el DOI del depósito de la **v1.2.0**, `10.5281/zenodo.22883012`, verificado contra DataCite: título correcto, `version: v1.2.0`, tipo Software, y `IsSupplementTo` apuntando a `github.com/gleiston-guerrero/SBVIA-AppWeb/tree/v1.2.0` — el repositorio de entrega, no un fork.
...
- **Gestión de la firma.** El criterio pide el SRS con la firma de aprobación del docente-director, y es el único punto de la rúbrica que **no depende del equipo**. Se declara aquí lo que sí se puede acreditar:
  - **El entregable está terminado.** `docs/requisitos/SRS-v1.3.0.pdf`, 24 páginas, compila con 0 errores y su texto coincide con una compilación nueva (comprobado en Piso 2). No lleva hoja de firmas propia a proposito: el docente-director emite su propio documento de aprobación con firma digital, de modo que una hoja de firmas dentro de este SRS estaria de mas y podria leerse como si se le pidiera firmar en un formato ajeno. El documento esta completo y no tiene observaciones pendientes de resolver por el equipo.
  - **La solicitud se ha realizado** por vía directa al docente-director, con el PDF adjunto y oferta de entrega impresa para firmarlo en persona.
  - **Lo que falta es, exactamente, su firma.** No hay ninguna acción pendiente del equipo.
  - **Por qué se documenta:** una firma que no se ha emitido no puede acreditarse, pero el expediente sí puede acreditar que el documento está completo y que la solicitud existe. La ausencia de firma se declara como condición externa y no se sustituye por ninguna inferencia, que es precisamente el defecto que la observación X6 señaló.
