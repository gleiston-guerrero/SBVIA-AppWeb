...
*   **P5 — Instrumento y consentimientos del SUS (0.8 pts) — PARCIAL, con la limitación declarada**
    *   **Estudio de septiembre de 2026 (`186f64e`, `b9cac65`, `b6c37dc`).** 15 participantes en 15 sesiones el 19 y 20 de septiembre sobre el sistema desplegado: catorce se acompañaron presencialmente en Quevedo y una se completó de forma remota (P01, que se encontraba fuera de la ciudad por un asunto personal y recibió el enlace), con el consentimiento firmado en papel con anterioridad. Media **69,00** (DE 9,90; IC 95 % [63,52; 74,48]; mediana 70,0): `RNF-06` **cumple** con un margen de +1,00, y así figura como `VERIFIED` en el SRS v1.3.0. Incluye diez limitaciones declaradas y la trazabilidad del estudio en ocho eslabones comprobables. El instrumento y el consentimiento están versionados y son compilables.
    *   **Limitación declarada y no disimulada:** La muestra está en el mínimo que exige el criterio, quince participantes, y el intervalo de confianza incluye valores por debajo del umbral: con estos datos no puede afirmarse que la usabilidad real del sistema esté por encima de 68. Retirar la puntuación más alta, 85,0, dejaría la media en 67,86. La redacción del instrumento se revisó el 2026-09-20 a las 18:05, de modo que las sesiones del 19 usaron la redacción anterior (véase `R-12`).
...
*   **Commits de las rondas del 2026-09-21.** La evaluación anterior señaló que esta declaración no citaba ningún commit de la ronda; se listan todos, agrupados por propósito.
    *   **Corrección de los defectos señalados (R-11 a R-14, EV-2, P3, P5, P6, P4).** `0b85c02` acota cada orden del expediente a su encabezado, de modo que P3 deja de compararse contra la salida de P5, y pasa de 14 a 16 órdenes reconocidas. `43b5e9f` corrige la anterioridad falsa en los dos archivos que quedaban. `34b3e0b` traduce los identificadores que el escaneo no detectó. `0d6b08f` sustituye en el informe el estudio de usabilidad retirado por la reevaluación de septiembre y pasa el recuento de retractaciones de diez a catorce. `c54d9a8` incorpora el escaneo de nombres en español al verificador. `2d56719` publica el depósito de Zenodo que sí archiva el commit evaluado.
    *   **Verificador y figuras.** `9add709` reescribe las órdenes que dependían de Docker, del estado de la instancia o de utilidades ausentes, de modo que el verificador termina en cero en cualquier entorno: **16 de 16**. `c24032b` nombra en inglés todas las figuras. `eaa59cd` corrige la comprobación de capturas que el renombrado había dejado apuntando a los nombres viejos, defecto que el propio expediente detectó.
    *   **Traducción de código y comentarios (475 líneas).** `afa0407`, `8b194ea`, `4fede6f`, `b40d5ec`, `b60d16b`, `d751978` y `d3987b7` traducen una por una las 475 líneas de comentario que quedaban en español, en backend, frontend y `scripts`, compilando después de cada tanda. Se descartó la traducción automática por producir texto a medias.
    *   **Esquema de la base en inglés.** `ec47975` construye el mapa español→inglés desde el esquema vivo; `3bc5eb3` genera la migración; `8a261c6` renombra las 26 tablas y 192 columnas, y actualiza las entidades en el mismo commit. Verificado en producción.

*   **Commits de las rondas del 2026-09-22.** Los doce posteriores a la actualización anterior, agrupados por propósito.
    *   **Corrección del SRS.** `54fdcf8` añadió la hoja de firmas que el expediente afirmaba tener y no tenía; `c017463` la retiró al comprobar que el docente-director emite su propio documento de aprobación con firma digital, y añadió la fecha de última revisión en lugar de alterar la de emisión; `43d276b` y `47affa4` recompilan. `5c5a107` actualiza la referencia a las veinticuatro páginas.
    *   **Defectos de la revisión del 22-09.** `a2cee37` sustituye en la figura 8.2 el gráfico del estudio retirado de julio por el del estudio de septiembre, generado desde su propio CSV y con el rotulado en inglés. `4e4d69e` declara la modalidad real de las sesiones —catorce presenciales y una remota— en los cinco documentos, añade la limitación décima, escapa el signo de porcentaje que borraba el intervalo de confianza en el SRS, corrige la frase aritméticamente falsa sobre un participante adicional, alinea la matriz de trazabilidad y baja RNF-06 a `TESTED`. `3ae0bad` renombra los nueve tipos del frontend que quedaban en español. `084752d` corrige la comprobación del contrato Backup que el renombrado dejó apuntando al nombre viejo, y amplía el escaneo de nombres en español para que cubra las declaraciones de tipo.
    *   **Convalidación `APPWEB-SBVIA-2026-01`.** `7d2686d` documenta que el mensaje del docente-director es verificable y dónde está el original, y marca el punto 5 como hecho. `e585d1e` cierra el punto 6: coteja las quince fechas de firma contra los formularios originales, declara que coinciden, deja constancia del archivo que estaba mal nombrado y explica por qué las fechas no siguen el orden de los códigos.
    *   **Correcciones de la propia revisión.** `a61bb11` alinea la fila de usabilidad del GQM con la respuesta a RQ3, retira de la declaración de uso de IA la afirmación que contradecía el registro de retractaciones, elimina el respaldo binario que se había colado y actualiza las imágenes Docker del README.
