...
\begin{titlepage}
    ...
    \footnotetext{Nota aclaratoria de autoría: El tercer integrante originalmente propuesto, Carlos Zamora Bumbila, no participó en la elaboración ni desarrollo de este proyecto, por lo que ha sido excluido del informe final y los metadatos del repositorio.}
...
\begin{table}[h]
    ...
    \begin{tabularx}{\textwidth}{@{}|l|l|X|l|@{}}
        ...
        1.2.0 & 18-09-2026 & Corrección de 14 observaciones del informe de revisión (N1–N14) & Pendiente de revisión \\ \hline
    \end{tabularx}
\end{table}

\textit{Nota: El informe de revisión firmado (\texttt{Revision\_SRS\_SBVIA\_v1.1.0.pdf}) declara explícitamente que
``no asigna calificación y no constituye aprobación del SRS''. Los estados anteriores reflejan que fueron
sometidos a revisión con observaciones, no aprobados.}
...
Este documento establece los requisitos verificables de SBVIA v1.2.0 y constituye la línea base contractual para desarrollo, prueba, despliegue y evaluación académica. Esta versión corrige las 14 observaciones del informe de revisión emitido por el docente-director sobre la v1.1.0.
...
\begin{table}[h]
    ...
    \begin{tabularx}{\textwidth}{@{}|l|X|@{}}
        ...
        Zenodo & Repositorio de publicación de software DOI \texttt{10.5281/zenodo.22740480}. \\ \hline
    \end{tabularx}
\end{table}
...
\begin{itemize}[label=\textbf{--}]
    ...
    \item \textbf{Anonimización:} Las métricas de evaluación (SUS) se publican únicamente en forma anonimizada con códigos P01–P15. El estudio SUS en sí está retirado y no se reportan resultados agregados (véase \texttt{docs/mediciones/sus/sus-analysis.md}).
\end{itemize}
...
Cobertura de pruebas automatizadas: $\geq$ 70\% de líneas (JaCoCo). Puntuación SUS media: no se reporta (estudio retirado; véase RNF-06). Lighthouse accesibilidad: 0.91 (color-contrast corregido en v1.2.0).
...
\begin{requisito}{RF-16 - Cierre de simulaciones vencidas}
    ...
    \textbf{Verificación:} Prueba de Integración (SimulationRepositoryTest.java) \quad \textbf{SQL:} db/procs/sp\_cerrar\_simulaciones\_vencidas.sql
\end{requisito}
...
\begin{requisito}{RF-17 - Generación de código de reporte}
    ...
    \textbf{Verificación:} Prueba de Integración (SimulationRepositoryTest.java) \quad \textbf{SQL:} db/procs/sp\_generar\_codigo\_reporte.sql
\end{requisito}
...
\begin{requisito}{RF-18 - Generación de reporte de simulación general}
    ...
    \textbf{Verificación:} Prueba de Integración (SimulationRepositoryTest.java) \quad \textbf{SQL:} db/procs/sp\_generar\_reporte\_simulacion.sql
\end{requisito}
...
\begin{requisito}{RF-19 - Generación de resumen de usuario}
    ...
    \textbf{Verificación:} Prueba de Integración (SimulationRepositoryTest.java) \quad \textbf{SQL:} db/procs/sp\_resumen\_usuario.sql
\end{requisito}
...
\begin{requisito}{RF-20 - Validación general de simulación}
    ...
    \textbf{Verificación:} Prueba de Integración (SimulationRepositoryTest.java) \quad \textbf{SQL:} db/procs/sp\_validar\_simulacion.sql
\end{requisito}
...
\begin{requisito}{RNF-06 - Escala de Usabilidad del Sistema SUS $\ge$ 68 puntos}
    \textbf{Tipo:} USAB \quad \textbf{Prioridad:} Must \quad \textbf{Estado:} NOT VERIFIED \\[0.2cm]
    ...
    \textbf{Criterio de aceptación:} La evaluación empírica SUS con al menos 15 participantes alcanza una puntuación media calculada igual o superior a 68. \textbf{No se reporta resultado}: el estudio SUS se retira porque su fecha declarada de aplicación (2026-07-28/29) es anterior a la incorporación al repositorio de la funcionalidad de simulación que dice haber evaluado (commit \texttt{55201f5}, 2026-09-03, y commit \texttt{5e852c6}, 2026-09-04). Las respuestas crudas se conservan sin garantía de validez metodológica. \\[0.2cm]
...
\begin{table}[h]
    ...
    \begin{tabularx}{\textwidth}{@{}|l|l|X|l|@{}}
        ...
        sp\_cerrar\_simulaciones\_vencidas & PROCEDURE & Huérfano (sin RF) & Pendiente vincular o eliminar \\ \hline
        sp\_generar\_codigo\_reporte & PROCEDURE & Huérfano (sin RF) & Pendiente vincular o eliminar \\ \hline
        sp\_generar\_reporte\_simulacion & PROCEDURE & Huérfano (sin RF) & Pendiente vincular o eliminar \\ \hline
        sp\_resumen\_usuario & PROCEDURE & Huérfano (sin RF) & Pendiente vincular o eliminar \\ \hline
        sp\_validar\_simulacion & PROCEDURE & Huérfano (sin RF) & Pendiente vincular o eliminar \\ \hline
    \end{tabularx}
\end{table}

\textit{Nota: Los 5 procedimientos marcados como ``Huérfano'' existen en la migración V14 pero no son invocados por ningún servicio Java ni cubiertos por ningún requisito formal. Se deberán vincular a un RF o eliminar en la próxima iteración.}
...
\begin{itemize}[label=\textbf{--}]
    ...
    \item \textbf{AC-06.} El repositorio contiene documentación, datos, metadatos y etiqueta v1.1.0 coherentes con el commit declarado. El repositorio de entrega es \texttt{gleiston-guerrero/SBVIA-AppWeb}.
\end{itemize}
...
\begin{longtable}{|p{1.6cm}|p{4.5cm}|p{3cm}|p{1.5cm}|p{2cm}|}
    ...
    RF-16 & Cerrar simulaciones (SP) & SimulationRepositoryTest & IMPLEMENTED & - \\ \hline
    RF-17 & Código de reporte (SP) & SimulationRepositoryTest & IMPLEMENTED & - \\ \hline
    RF-18 & Reporte simulación (SP) & SimulationRepositoryTest & IMPLEMENTED & - \\ \hline
    RF-19 & Resumen usuario (SP) & SimulationRepositoryTest & IMPLEMENTED & - \\ \hline
    RF-20 & Validar simulación (SP) & SimulationRepositoryTest & IMPLEMENTED & - \\ \hline
    ...
    RNF-06 & SUS $\ge$68 & sus-analysis.md & NOT VERIFIED & — \\ \hline
