...
\begin{titlepage}
    ...
    \begin{table}[h]
        ...
        \begin{tabularx}{\textwidth}{|l|X|}
            ...
            \textbf{Punto del historial} & etiqueta \texttt{v1.1.0}, anotada sobre este commit \\ \hline
            \textbf{Esquema de la base} & 28 tablas, 192 columnas y 14 rutinas, todos con nombre en inglés desde \texttt{V19} y \texttt{V20} \\ \hline
            \textbf{DOI del software (Zenodo)} & \texttt{10.5281/zenodo.22906357} \\ \hline
            \textbf{Fecha de emisión} & 21 de septiembre de 2026 \\ \hline
            \textbf{Última revisión} & 22 de septiembre de 2026 \\ \hline
        \end{tabularx}
    \end{table}
...
\begin{table}[h]
    ...
    \begin{tabularx}{\textwidth}{@{}|l|X|@{}}
        ...
        Zenodo & Repositorio de publicación de software DOI \texttt{10.5281/zenodo.22906357}. \\ \hline
    \end{tabularx}
\end{table}
...
\begin{requisito}{RF-06 - Actualización masiva de conductores inactivos}
    ...
    \textbf{Nota (X8):} \texttt{AuditLog} es la entidad JPA que mapea la tabla física de auditoría; no son dos almacenes distintos. Esa tabla se creó como \texttt{bitacora\_auditoria} en la migración \texttt{V3\_\_auditoria\_operaciones.sql} y la migración \texttt{V17\_\_renombrar\_tablas\_ingles.sql} la renombró a \texttt{audit\_log}, que es el nombre declarado en \texttt{@Table(name = "audit\_log")} dentro de \texttt{AuditLog.java}. El procedimiento \texttt{sp\_actualizar\_usuarios\_inactivos} redefinido en \texttt{V18\_\_conectar\_procedimientos\_sp.sql} insertaba en el nombre anterior al renombrado (\texttt{bitacora\_auditoria}); \textbf{esa incoherencia está corregida}. La migración \texttt{V19\_\_renombrar\_esquema\_ingles.sql} vuelve a crear los procedimientos escribiendo en \texttt{audit\_log} con las columnas en inglés, y \texttt{V20\_\_reparar\_funciones\_trigger.sql} repara las cinco funciones de disparador que el patrón de V19 no reconoció, porque V1 es un volcado y las declara con otra forma. Ninguna migración ya aplicada se modificó —Flyway valida su \textit{checksum}—: la corrección se hizo con migraciones nuevas, que es la forma correcta de hacerlo. \\[0.2cm]
...
\begin{requisito}{RNF-06 - Escala de Usabilidad del Sistema SUS $\ge$ 68 puntos}
    ...
    \textbf{Criterio de aceptación:} La evaluación empírica SUS con al menos 15 participantes alcanza una puntuación media calculada igual o superior a 68. \textbf{Resultado:} la reevaluación se ejecutó el 19 y 20 de septiembre de 2026 con \textbf{15 participantes} en 15 sesiones presenciales sobre el sistema desplegado, con los diez ítems del cuestionario SUS estándar de Brooke, cuya redacción en español se revisó el 2026-09-20 a las 18:05 (commit \texttt{b9cac65}), de modo que las sesiones del 19 de septiembre usaron la redacción anterior a esa revisión, lo que se declara como limitación. La media es \textbf{69,00} (DE 9,90; IC 95\,\% [63,52; 74,48]; mediana 70,0), de modo que \textbf{el criterio se cumple con un margen de +1,00}. El estudio de julio de 2026 permanece retirado y no se emplea como evidencia. Análisis, limitaciones declaradas y trazabilidad en \texttt{docs/mediciones/sus/sus-analysis-2026-09.md}. \\[0.2cm]
    \textbf{Cierre del plan de reevaluación (X12):} el plan queda \textbf{cumplido}. El requisito se mantiene vigente y con prioridad \textit{Must}. Se declaran sus limitaciones, entre ellas que la media se sitúa a 1,00 punto del umbral y que la muestra está en el mínimo exigido, quince participantes, de modo que un participante adicional con la media observada habría dejado la puntuación por debajo del criterio. \\[0.2cm]
...
\begin{longtable}{|p{1.6cm}|p{4.5cm}|p{3cm}|p{1.5cm}|p{2cm}|}
    ...
    RNF-06 & SUS $\ge$68 & sus-analysis-2026-09.md & TESTED & — \\ \hline
