...
\begin{table}[h]
    ...
    \begin{tabularx}{\textwidth}{@{}|l|l|X|l|@{}}
        ...
        1.1.0 & 18-09-2026 & Correcciones finales (contrato REST, Lighthouse, URLs y limpieza) & Revisada con observaciones \\ \hline
    \end{tabularx}
\end{table}

\vspace{0.3cm}
\textit{Nota: el informe de revisión firmado por el docente-director (\texttt{Revision\_SRS\_SBVIA\_v1.1.0.pdf}) declara literalmente que ``no asigna calificación y no constituye aprobación del SRS''. Su firma acredita que el documento fue sometido a revisión, no que haya sido aprobado. Por ese motivo esta versión consta como revisada con observaciones y no como aprobada: la aprobación del SRS se emite por escrito y en el propio documento, no por inferencia de una confirmación verbal.}

\vspace{0.3cm}
\textit{Nota sobre la etiqueta: este documento describe el estado del proyecto en la ronda de correcciones del contrato REST, Lighthouse y URLs, y su contenido no cambia después de esa ronda. La etiqueta \texttt{v1.1.0} es, además, la \textbf{línea base del examen suspenso} y, por autorización expresa del docente-director \textbf{constante por escrito en su evaluación del 2026-09-20}, se sigue actualizando (reetiquetando) con cada ronda de correcciones posterior; por tanto la etiqueta \texttt{v1.1.0} no identifica únicamente el contenido de esta versión del documento. La política general de inmutabilidad de etiquetas y esta excepción, acotada a \texttt{v1.1.0}, están documentadas en \texttt{docs/VERSIONING.md}.}
...
\begin{requisito}{RNF-06 - Escala de Usabilidad del Sistema SUS $\ge$ 68 puntos}
    ...
    \textbf{Nota (corrección posterior a la revisión):} El estudio SUS se retira: su fecha declarada de aplicación (2026-07-28/29) es anterior a la incorporación al repositorio de la funcionalidad de simulación que dice haber evaluado (commits \texttt{55201f5} y \texttt{5e852c6}, 2026-09-03/04). No se reporta puntuación agregada. Este requisito pasa a NOT VERIFIED sin alterar el resto del contenido revisado. \\[0.2cm]
