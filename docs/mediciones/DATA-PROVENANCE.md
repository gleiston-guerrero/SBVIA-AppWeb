...
| **Tabla 5 (Cap. 8) — RETIRADA** | Resumen de Usabilidad SUS (N=15): eliminada del informe al retirarse el estudio | `docs/mediciones/sus/sus-raw-data.csv` | — (no se deriva resultado) | — |
| **Tabla 6 (Cap. 8)** | Cobertura de Código JaCoCo | `docs/mediciones/jacoco/coverage-summary.csv` | `mvn verify` (JaCoCo plugin) | `13ed0b1` |
| **Tabla 7 (Cap. 8)** | Métricas Lighthouse (Mobile & Desktop) | `docs/mediciones/lighthouse/mobile/*.report.json` y `docs/mediciones/lighthouse/desktop/*.report.json` (3 corridas por perfil, `manifest.json` en cada carpeta) | `node scripts/lighthouse-collect.js` (Lighthouse 12.1.0) | `2ad52ff` |
...
| **Figura 3 (Cap. 8)** | Distribución de las respuestas crudas SUS — conservada solo como registro gráfico; estudio retirado | `docs/mediciones/sus/sus-raw-data.csv` | `scripts/gen-figuras.py` | `0d513c4` |
