...
\caption{Modelo Entidad-Relación de la aplicación SBVIA. Fuente: Elaboración propia.}
\label{fig:er-model}
\end{figure}
...
\caption{Diagrama de Contenedores C4 (Nivel 2) para SBVIA. Fuente: Elaboración propia.}
\label{fig:c4-level2}
\end{figure}
...
\caption{Diagrama de Clases UML para los componentes principales. Fuente: Elaboración propia.}
...
\midrule
\textbf{G2. Usabilidad:} Medir la facilidad de uso percibida del prototipo & ¿Es el sistema considerado usable por los conductores novatos? & Puntuación individual y media SUS (umbral $> 68$) & Cuestionario SUS \\
\midrule
...
Dado que este estudio evalúa simultáneamente el rendimiento (latencia en frío vs caliente) y la usabilidad (puntuación SUS frente a un umbral), se realizan múltiples pruebas de hipótesis estadísticas sobre el mismo experimento. Para controlar la Tasa de Error por Familia (FWER) y reducir el riesgo de falsos positivos (Error Tipo I), se aplica el procedimiento de corrección paso a paso de Holm-Bonferroni \cite{holm1979simple}. De este modo, los valores $p$ obtenidos se ordenarán de menor a mayor y se compararán contra niveles de significancia ajustados progresivamente ($\alpha / k, \alpha / (k-1), \dots$) antes de rechazar cualquier hipótesis nula.
...
\begin{figure}[htbp]
    ...
    \caption{Diagrama de Contexto C4 (Nivel 1) para SBVIA. Fuente: Elaboración propia.}
    \label{fig:c4-level1}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \caption{Diagrama de Componentes C4 (Nivel 3) para el Backend. Fuente: Elaboración propia.}
    \label{fig:c4-level3}
\end{figure}
...
El patrón Modelo-Vista-Controlador (MVC) dicta cómo fluye la informacin a través de la aplicación. La Figura~\ref{fig:mvc-flow} ilustra este flujo en el entorno Spring Boot: las peticiones HTTP del cliente son recibidas por el Controlador, el cual delega la lógica al Servicio. El Servicio manipula el Modelo y los DTOs interactuando con la base de datos a través de los Repositorios. Finalmente, la respuesta viaja de vuelta al cliente.
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{diagramas/mvc-flow-springboot-en.png}
    \caption{Diagrama de Flujo MVC en Spring Boot. Fuente: Elaboración propia.}
    \label{fig:mvc-flow}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \caption{Dashboard principal con acceso a escenarios, prácticas e historial.}
    \label{fig:pantalla2}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \caption{Pantalla core para la selección y configuración de escenarios de conducción.}
    \label{fig:pantalla3}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \caption{Reporte detallado de retroalimentación generado por la IA al finalizar la práctica.}
    \label{fig:pantalla4}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \caption{Distribución de latencia comparativa de las ejecuciones de k6. Fuente: Elaboración propia.}
...
As depicted in Figure~\ref{fig:sus-distribution}, la línea de media coincide con 84.17 y permite comprobar visualmente que todos los participantes superaron el umbral de aceptación de 68 puntos.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.96\textwidth]{diagramas/sus-boxplot.png}
    \caption{Distribución y puntuaciones individuales de la escala SUS para 15 participantes. Fuente: elaboración propia.}
...
\textbf{Disponibilidad de Código:} El código fuente está disponible públicamente en \url{https://github.com/keithdrox/SBVIA-AppWeb}. La preservación persistente del software se realizó en Zenodo bajo el DOI \texttt{10.5281/zenodo.22740480}, siguiendo los principios de citación de software \cite{smith2016software}.
