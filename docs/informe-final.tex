...
\begin{titlepage}
    ...
    DOI Software (Zenodo): \url{https://doi.org/10.5281/zenodo.22740480} \\
...
\begin{table}[htbp]
\centering
\caption{Aplicación del enfoque Goal-Question-Metric (GQM) en la evaluación de SBVIA}
\label{tab:gqm}
\footnotesize
...
\midrule
\textbf{G2. Usabilidad:} Medir la facilidad de uso percibida del prototipo & ¿Es el sistema considerado usable por los conductores novatos? & Puntuación individual y media SUS (umbral $> 68$). \textbf{No se reporta: estudio retirado} & Cuestionario SUS (no aplicado) \\
\midrule
...
\begin{itemize}
    ...
    \item \textbf{Usabilidad:} Cuestionario SUS de 10 ítems \cite{brooke1996sus} con $N = 15$ respuestas registradas. La aplicación se \textbf{retira} como evidencia empírica (véase la sección de Usabilidad) y el consentimiento informado no está verificado.
...
Para usabilidad se declaró $H_{0U}$: la media poblacional SUS no supera el umbral de 68, frente a $H_{1U}$, que lo supera. \textbf{Esa hipótesis no se evalúa en este informe}: el estudio SUS se retira por no poder verificarse contra el historial de desarrollo del sistema (véase la sección de Usabilidad), de modo que no se reportan media, mediana, desviación típica ni intervalo de confianza como resultado de la prueba. Para cobertura, seguridad y Lighthouse se aplican criterios de aceptación previamente fijados y no pruebas de hipótesis retrospectivas.

La familia de hipótesis se declaró contemplando dos pruebas sobre el mismo experimento (rendimiento frío/caliente y usabilidad SUS frente al umbral de 68). Al retirarse el estudio SUS, en este informe solo permanece la comparación de rendimiento como prueba de hipótesis efectivamente evaluable; el procedimiento de Holm-Bonferroni se mantiene documentado para el caso en que se incorporen nuevas pruebas. De aplicarse, los valores $p$ obtenidos se ordenarían de menor a mayor y se compararían contra niveles de significancia ajustados progresivamente ($\alpha / k, \alpha / (k-1), \dots$) antes de rechazar cualquier hipótesis nula \cite{holm1979simple}.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.94\textwidth]{diagramas/pantalla-1---inicio-de-sesin-registro.png}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.94\textwidth]{diagramas/pantalla-2---dashboard-principal.png}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.94\textwidth]{diagramas/pantalla-3---seleccin-de-escenario-core.png}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.94\textwidth]{diagramas/pantalla-4---reporte-de-retroalimentacin.png}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.88\textwidth]{diagramas/k6-latencia-comparativa.png}
    \caption{Comparative latency distribution of the k6 runs. Source: Own elaboration.}
    \label{fig:k6-boxplot}
\end{figure}

\section{Usabilidad del Sistema (SUS) — estudio retirado}
\label{sec:sus-retirado}
...
En consecuencia, las respuestas de \texttt{docs/mediciones/sus/sus-raw-data.csv} se presentan \textbf{sin garantía de validez metodológica} y \textbf{no deben interpretarse como evidencia empírica confiable del sistema}. Tampoco puede confirmarse el consentimiento informado de los participantes (véase \texttt{docs/etica/consentimientos/registro-aceptacion.md}, \texttt{docs/mediciones/sus/sus-analysis.md} y \texttt{ETHICS.md} \S iii).

La Figura~\ref{fig:sus-distribution} se conserva únicamente como registro gráfico de los datos crudos. \textbf{No se reportan} media, desviación estándar ni intervalo de confianza como resultados válidos, ni se clasifica al sistema según escalas de interpretación del SUS.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.96\textwidth]{diagramas/sus-boxplot.png}
    \caption{Individual SUS raw responses from 15 participants (study retracted; shown as raw material only, without aggregated statistics). Source: Own elaboration.}
...
\begin{itemize}
    ...
    \item \textbf{Respuesta a RQ3:} Esta pregunta de investigación no pudo responderse con evidencia empírica válida: el instrumento de usabilidad disponible en el repositorio no puede verificarse contra el historial de desarrollo del sistema (véase \texttt{docs/etica/consentimientos/registro-aceptacion.md} y \texttt{docs/mediciones/sus/sus-analysis.md}).
\end{itemize}
...
El estudio SUS se retira y por tanto no se triangula con Lighthouse ni con ninguna otra medición. La evaluación de usabilidad de este proyecto queda limitada a la auditoría Lighthouse, que inspecciona propiedades técnicas de una página y su configuración de carga, no la percepción del usuario. No se dispone de evidencia empírica sobre la usabilidad autopercibida de la interfaz, y esa ausencia se declara como tal en lugar de sustituirse por una medición distinta.
...
\begin{itemize}
    ...
    \item \textbf{Validez Externa:} La evaluación empírica con personas se retira (véase la sección de Usabilidad), por lo que no se sostiene ninguna afirmación sobre la composición de la muestra ni sobre la generalización de sus resultados. Se reconoce la necesidad de realizar ensayos de usabilidad con participantes caracterizados explícitamente y en escuelas de conducción de distintas provincias.
...
Usabilidad (retirada) & Respuestas SUS anónimas & Sin resultado derivado: estudio retirado \\
