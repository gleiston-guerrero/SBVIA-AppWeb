...
| **Commit de corrección** | `778354b` — 21-09-2026 — *fix(integrity): correct the defects of the 21 September review (R-11 to R-14)* |
...
### R-12 — Anterioridad del instrumento SUS afirmada en cinco archivos

| Campo | Detalle |
|:---|:---|
| **Qué se retractó** | Cinco archivos afirmaban que los diez ítems del SUS aplicados están versionados desde el commit `786e042` (2026-09-18) y que son **anteriores a la primera sesión** o **anteriores a toda sesión**: `docs/etica/ETHICS.md`, `docs/requisitos/SRS-v1.3.0.tex`, `docs/mediciones/sus/sus-analysis-2026-09.md`, `docs/etica/consentimientos/registro-aceptacion.md` y `docs/requisitos/CHANGELOG-REQ.md`. El historial real del instrumento (`git log -- docs/mediciones/sus/instrumento-sus.md`) muestra que su último cambio es `b9cac65`, del **2026-09-20 a las 18:05**, y que esa revisión **reescribe siete de los diez ítems**. Las sesiones del 19 de septiembre son anteriores a esa revisión. |
| **Corrección incompleta que se enmienda aquí** | Una primera corrección declaró corregidos **cuatro** archivos, y no era cierto: eran tres. En `registro-aceptacion.md` el reemplazo quedó **mal empalmado** —el párrafo nuevo se insertó delante del texto antiguo, que siguió leyéndose: *"(2026-09-18), anterior a la primera sesión (2026-09-19)"*— y `CHANGELOG-REQ.md`, que también lo afirmaba, **no figuraba siquiera en la retractación**. El error de método fue reemplazar una subcadena y dar por corregido el párrafo entero, sin leer el resultado. |
| **Commit de corrección** | `778354b` — 21-09-2026 — *fix(integrity): correct the defects of the 21 September review (R-11 to R-14)*, y el de esta ronda |
| **Qué se hizo** | Se corrigen los cinco archivos y se comprueba con una búsqueda real sobre todo el árbol que la afirmación no sobrevive en ninguno. El registro de consentimientos declara además la cronología del propio consentimiento sin conciliarla: generado el 2026-09-19 y compilado en PDF el **2026-09-20 a las 13:03**, posterior al inicio de las sesiones del 19. Las quince fechas de firma siguen marcadas como **por verificar**. |
| **Por qué** | La afirmación se apoyaba en una orden —`git log -- docs/mediciones/sus/instrumento-sus.md`— que en realidad la refuta. Y el cierre de la ronda anterior es el segundo caso seguido de declarar corregido algo que no resiste una comprobación literal. |

### R-13 — Cifra falsa en el mensaje del commit de renombrado

| Campo | Detalle |
|:---|:---|
| **Qué se retractó** | El mensaje del commit `cd30632` afirma: *"0 of 172 methods in main and 0 of 25 in test carry a Spanish name"*. Y el cierre de la ronda anterior lo repitió con otra cifra igualmente optimista: *"en `src/main` y en el frontend no queda ningún nombre en español y en `src/test` quedan tres"*. **Ninguna de las dos se sostiene.** |
| **Medición real** | Un escaneo de tipos y métodos —Java de `main` y `test`, y TypeScript— con una lista amplia de palabras españolas marca **14 identificadores**. Los 14 están justificados y ninguno es renombrable: **3 impuestos por el contrato JSON** (`getFechaProgramada`, `setFechaProgramada`, `semaforosRespetados`), **3 impuestos por el esquema**, que son *derived queries* sobre campos de entidad (`findByActivoTrue`, `findByCodigo`, `findFirstByActivoTrueOrderByIdVehiculoAsc`), **3 nombres de prueba que reflejan a propósito esos métodos de repositorio** (`findByActivoTrue_returnsOnlyActiveScenarios`, `findByCorreoAndExistsByCorreo_resolveAuthentication`, `findByUsuarioAndIdUsuario_listsUserSimulations`) y **5 falsos positivos** por palabras compartidas con el inglés (`catchError` ×2, `showError` y `getErrorMessage`, por *error*; `finalScoreIs`, por *final*). **Renombrables de verdad: 0.** |
| **Qué se hizo** | Se traducen los identificadores que sí eran renombrables: **13 en el frontend** (`formatearBytes`, `getTodas`, `getRetroalimentacion`, `editar`, `dibujarSenalVelocidad`, `vacio`, `pausar`, `reanudar`, `centroCarril`, `carrilCercano`, `dibujar`, `dibujarAuto`, `dibujarSemaforoEn`) y **11 en `src/test`** (`servicioReal`, `simulacionPropia`, `persistTipoVia`, `persistNivelDificultad`, `persistTipoClima`, `buildTipoVia`, `buildNivelDificultad`, `buildTipoClima`, `buildEscenario`, `respuesta`, `servicio`). Se comprueba con una búsqueda literal que los nueve nombres concretos señalados en la evaluación devuelven **cero** ocurrencias. El escaneo queda incorporado al verificador de integridad, de modo que la afirmación deja de depender de un recuento mío. |
| **Por qué** | Es la **segunda ronda seguida** en que este punto se declara cerrado con un recuento que no se sostiene, y con el mismo error de fondo: medir con una lista de palabras incompleta y no comprobar el resultado. En esta ronda, además, el renombrado se hizo con un patrón que excluía las llamadas del tipo `objeto.metodo(`, de modo que la primera pasada dejó el frontend sin compilar; lo detectó la compilación, no una revisión mía. |
| **Commit de corrección** | `778354b` — 21-09-2026 — *fix(integrity): correct the defects of the 21 September review (R-11 to R-14)* — y el de esta ronda |
| **Verificación** | `ng build` limpio y `mvn test` con **113 pruebas, 0 fallos y 0 errores**. |
...
| R-12 | Anterioridad del instrumento SUS afirmada en cinco archivos | esta ronda | 22-09-2026 |
