...
| **Commit de corrección** | El de esta ronda — 21-09-2026 — *docs(integrity): ...* |
| **Qué se hizo** | Se mide de nuevo con un patrón que cubre los métodos sin modificador de acceso, los anidados y el frontend, y se completa el renombrado: 6 métodos de repositorio y servicio, 25 en el frontend con sus llamadas en plantilla, y **116 nombres de método de prueba** (119 ocurrencias). Cifra final declarada: en `src/main` no queda ningún nombre en español que no esté impuesto por el esquema o por el contrato JSON, y en `src/test` quedan **3**, que son los de `RepositoryIntegrationTest` y reflejan deliberadamente los nombres de los métodos de repositorio que prueban. El mensaje de `cd30632` no se reescribe: queda en el historial y esta entrada es su corrección. |
| **Por qué** | Es una cifra en el historial del repositorio que su propia comprobación no respalda. El recuento original usó un patrón que exigía `public`, `private` o `protected` al comienzo de la línea, de modo que omitía los métodos de paquete, los anidados y los repartidos en varias líneas, y ni siquiera miraba `src/test` ni el frontend. Verificado tras el renombrado: `mvn test` ejecuta **113 pruebas con 0 fallos y 0 errores**. |
